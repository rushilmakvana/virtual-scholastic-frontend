...
const getCourseDetailsAndPopulateUI = (courseId) => {
  ...
  fetch(`http://localhost:3000/courses/getCourseDetails?courseId=${courseId}`, {
    method: "GET",
    headers: {
    ...
    .then((response) => {
      if (response) {
        ...
        window.localStorage.setItem(
          "ENROLLED_STUDENTS",
          JSON.stringify(response.courseDetails.ENROLLED_STUDENTS)
        );
  ...
  fetch(`http://localhost:3000/courses/getAllotedCourses`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      Authorization: `${token}`,
...
const deleteStudent = (event) => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      fetch("http://localhost:3000/courses/removeStudentFromCourse", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `${token}`,
...
document
  ...
  .addEventListener("click", () => {
    ...
    if (studentId.length < 1) {
      ...
      const enrolledStudents = JSON.parse(
        localStorage.getItem("ENROLLED_STUDENTS")
      );
      if (
        ...
        document.getElementById("add-student-submit-btn").innerText =
          "Please wait...";
        fetch("http://localhost:3000/courses/addStudentToCourse", {
          method: "POST",
          headers: new Headers({
            "Content-Type": "application/json",
            Authorization: token,
